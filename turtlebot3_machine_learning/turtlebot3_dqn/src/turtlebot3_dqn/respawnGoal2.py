...
class Respawn():
    def __init__(self):
        ...
        self.modelName = 'goal'
        self.obstacle_1 = -5.3, 4.4
        self.obstacle_2 = -0.3, 4.1
        self.obstacle_3 = 3.4, 3
        self.obstacle_4 = -3.4, -0.1
        self.obstacle_5 = 2.66, -0.1
    ...
    def getPosition(self, position_check=False, delete=False):        #position_check，检查新的目标点是否与障碍物碰撞，False，取初始化目标点，True，检查
        if delete:
            self.deleteModel()

        if self.stage != 4:
            while position_check:
                goal_x1 = random.randrange(0, 15) / 10.0
                goal_x2 = random.randrange(-8, 0) / 10.0
                goal_x3 = random.randrange(-50, -15) / 10.0
                goal_xx=[goal_x1,goal_x2,goal_x3]
                goal_number= random.randrange(0, 2)
                goal_x=goal_xx[goal_number]
                ...
                if abs(goal_x - self.obstacle_1[0]) <= 0.5 and abs(goal_y - self.obstacle_1[1]) <= 0.5:
                    ...
                    position_check = True
                elif abs(goal_x - self.obstacle_4[0]) <= 1 and abs(goal_y - self.obstacle_4[1]) <= 1:
                    position_check = True
                elif abs(goal_x - self.obstacle_5[0]) <= 1 and abs(goal_y - self.obstacle_5[1]) <= 1:
                    position_check = True
