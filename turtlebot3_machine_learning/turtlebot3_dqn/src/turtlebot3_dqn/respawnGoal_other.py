    ...
    def getPosition(self, position_check=True, delete=False):        #position_check，检查新的目标点是否与障碍物碰撞，False，取初始化目标点，True，检查 ############e
        ...
        if self.stage != 4:
            while position_check:
                d3qn_her=np.load('goal_collision.npy')
                ...
                if goal_num == 1:
                   goal_x = random.randrange(2, 31) / 10.0
                   goal_y = random.randrange(9, 21) / 10.0
                if goal_num == 2:
                   goal_x = random.randrange(-17, -4) / 10.0
                   goal_y = random.randrange(-8, 3) / 10.0
                if goal_num == 3:
                   goal_x = random.randrange(1, 31) / 10.0
                   goal_y = random.randrange(-27, -15) / 10.0
                if goal_num == 4:
                   goal_x = random.randrange(1, 13) / 10.0
                   goal_y = random.randrange(-10,4) / 10.0
