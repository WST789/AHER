...
from respawnGoal_num import Respawn
...
class Env():
    ...
    def getGoalDistace(self):                                               #返回目标距离
        goal_distance = round(math.hypot(self.goal_x - self.position.x, self.goal_y - self.position.y), 2)                    ##初始小车位置是(0,0)？
    ...
    def getState(self, scan):                                        #获取状态，由激光数据获取状态
        ...
        return scan_range + [heading, current_distance,self.goal_x,self.goal_y], done     ##heading, current_distance,goalx,goaly加到scan_range[]后面
    ...
    def setReward(self, state, done, action):                   #设置奖励
        ...
        heading = state[-4]                                 ##
        done_d3qn = False                                        #done_d3qn  自己设置，回合修改条件
        ...
        ####################测试
        scan_range=state[:-4]
        if(0.13<abs(min(scan_range))<0.5):
            reward-=(0.5-abs(min(scan_range)))*3                                                 #远离障碍物*5   #################
            # reward=-(0.5-abs(min(scan_range)))*10 
            # print('zz')

        # reward = 0
        if(0.3<current_distance<0.8):
            
            reward+=(0.8-current_distance)*10
    ...
    def step(self, action):                                         
       ...
       # if done | done_d3qn:
        ...
        yaw = self.yaw
        return np.asarray(state), reward, done,done_d3qn,position,yaw                              # #返回下一个状态，奖励，done ，position是next_state对应的坐标,yaw用来计算heading
    ...
    def Studytime(self):                                                            ###############超过规定的学习回合数，所以重新给定目标点
        self.pub_cmd_vel.publish(Twist())
        self.goal_x, self.goal_y = self.respawn_goal.getPosition(True, delete=True)   #给定新的目标点,新的目标距离
        self.goal_distance = self.getGoalDistace()
        self.get_goalbox = False
